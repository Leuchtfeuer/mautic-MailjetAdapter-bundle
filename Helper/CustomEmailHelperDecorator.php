...
class CustomEmailHelperDecorator extends MailHelper
{
    ...
    public function queue($dispatchSendEvent = false, $returnMode = self::QUEUE_RESET_TO): bool|array
    {
        ...
        $currentAddress = key($this->queuedRecipients);
        $name = $this->queuedRecipients[$currentAddress];
        if(isset($this->metadata[$key]['contacts'][$currentAddress])){
            $address = $this->getNewUniqueEmail($currentAddress,$key);
            unset($this->queuedRecipients[$currentAddress]);
